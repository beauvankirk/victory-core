...
// export default (WrappedComponent) => {
  ...
  methods: {
    getEvents () {
      const getScopedEvents = Events.getScopedEvents.bind(this)
      return partialRight(Events.getEvents.bind(this), getScopedEvents)
    },
    getEventState () {
      return Events.getEventState.bind(this)
    },
    setupEvents (props) {
      const { sharedEvents } = props
      const components = this.expectedComponents
    ...
    getBaseProps (props) {
      ...
      return isFunction(this.getBaseProps)
        ? this.getBaseProps(modifiedProps) : {}
    },
    ...
    // renderContinuousData(props) {
      ...
      const labelComponents = dataKeys.reduce((memo, key) => {
        const labelProps = this.getComponentProps(this.labelComponent, 'labels', key)
        if (labelProps && labelProps.text !== undefined && labelProps.text !== null) {
          memo = memo.concat(React.cloneElement(this.labelComponent, labelProps))
        }
        return memo
      }, [])
      const dataProps = this.getComponentProps(this.dataComponent, 'data', 'all')
      const children = [React.cloneElement(this.dataComponent, dataProps), ...labelComponents]
      return this.renderContainer(this.groupComponent, children)
    },
